@ProductsAtBanners_SmallElements_1Column_UnlimitedRow

Feature: Проверка отображения баннера с товарами - шаблон "Мелкие элементы", 1 колонка, Без ограничений

  Scenario: Создаём баннер с товарами
    Given Переходим на страницу "Маркетинг" -- "Баннеры"
    And Создаём новый баннер с товарами: "ProductsAtBanners_SmallElements_1Column_UnlimitedRow", "small_items", "1", "0", "ccffcc"
  ...
  Scenario: Создаём блок для баннера с товарами
    ...
    And Отключаем все блоки в секции, если в секции отсутствует блок "ProductsAtBanners_SmallElements_1Column_UnlimitedRow"
    And Добавляем в секцию новый блок типа "Баннеры" и переименовываем его на "ProductsAtBanners_SmallElements_1Column_UnlimitedRow"
    Given Получаем ID блока "ProductsAtBanners_SmallElements_1Column_UnlimitedRow"
    And Переходим в настройки блока "ProductsAtBanners_SmallElements_1Column_UnlimitedRow"
    And Выбираем шаблон блока "AB: Расширенный баннер" и нажимаем кнопку 'Настройки'
    And Добавляем в блок баннер "ProductsAtBanners_SmallElements_1Column_UnlimitedRow"
  ...
  Scenario: Работаем с витриной и выполняем проверки
    ...
    And Делаем скриншот "ProductsAtBanners_SmallElements_1Column_UnlimitedRow 01"
    And Переключаемся на "ar" язык интерфейса витрины
    And Скроллимся к блоку "баннера"
    And Делаем скриншот "ProductsAtBanners_SmallElements_1Column_UnlimitedRow 02(RTL)"