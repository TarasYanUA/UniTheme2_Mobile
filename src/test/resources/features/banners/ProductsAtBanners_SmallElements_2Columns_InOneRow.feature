@ProductsAtBanners_SmallElements_2Columns_InOneRow

Feature: Проверка отображения баннера с товарами - шаблон "Мелкие элементы", 2 колонки, В один ряд

  Scenario: Создаём баннер с товарами
    Given Переходим на страницу "Маркетинг" -- "Баннеры"
    And Создаём новый баннер с товарами: "ProductsAtBanners_SmallElements_2Columns_InOneRow", "small_items", "2", "1", "afd8fd"
  ...
  Scenario: Создаём блок для баннера с товарами
    ...
    And Отключаем все блоки в секции, если в секции отсутствует блок "ProductsAtBanners_SmallElements_2Columns_InOneRow"
    And Добавляем в секцию новый блок типа "Баннеры" и переименовываем его на "ProductsAtBanners_SmallElements_2Columns_InOneRow"
    Given Получаем ID блока "ProductsAtBanners_SmallElements_2Columns_InOneRow"
    And Переходим в настройки блока "ProductsAtBanners_SmallElements_2Columns_InOneRow"
    And Выбираем шаблон блока "AB: Расширенный баннер" и нажимаем кнопку 'Настройки'
    And Добавляем в блок баннер "ProductsAtBanners_SmallElements_2Columns_InOneRow"
  ...
  Scenario: Работаем с витриной и выполняем проверки
    ...
    And Делаем скриншот "ProductsAtBanners_SmallElements_2Columns_InOneRow 01"
    And Переключаемся на "ar" язык интерфейса витрины
    And Скроллимся к блоку "баннера"
    And Делаем скриншот "ProductsAtBanners_SmallElements_2Columns_InOneRow 02(RTL)"