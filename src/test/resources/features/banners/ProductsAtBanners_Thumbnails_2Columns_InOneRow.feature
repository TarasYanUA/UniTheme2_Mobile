@ProductsAtBanners_Thumbnails_2Columns_InOneRow

Feature: Проверка отображения баннера с товарами - шаблон "Миниатюры", 2 колонки, В один ряд

  Scenario: Создаём баннер с товарами
    Given Переходим на страницу "Маркетинг" -- "Баннеры"
    And Создаём новый баннер с товарами: "ProductsAtBanners_Thumbnails_2Columns_InOneRow", "links_thumb", "2", "1", "ccffcc"

  Scenario: Создаём блок для баннера с товарами
    Given Переходим на страницу "Веб-сайт" -- "Темы"
    Given Переходим на страницу "Макеты", что на странице 'Темы'
    Given Переходим во вкладку "Домашняя страница", что на странице 'Макеты'
    And Получаем ID секции с блоком "<mark>Возможно,</mark> вас это заинтересует"
    And Отключаем все блоки в секции, если в секции отсутствует блок "ProductsAtBanners_Thumbnails_2Columns_InOneRow"
    And Добавляем в секцию новый блок типа "Баннеры" и переименовываем его на "ProductsAtBanners_Thumbnails_2Columns_InOneRow"
    Given Получаем ID блока "ProductsAtBanners_Thumbnails_2Columns_InOneRow"
    And Переходим в настройки блока "ProductsAtBanners_Thumbnails_2Columns_InOneRow"
    And Выбираем шаблон блока "AB: Расширенный баннер" и нажимаем кнопку 'Настройки'
    And Добавляем в блок баннер "ProductsAtBanners_Thumbnails_2Columns_InOneRow"
    Then Сохраняем настройки блока

  Scenario: Работаем с витриной и выполняем проверки
    When Переходим на витрину
    Given Разавторизоваться на витрине
    Given Переключаемся на "ru" язык интерфейса витрины
    And Скроллимся к блоку с баннером
    And Выполняем проверки в блоке с баннером:
      | Проверяем, что шаблон у товаров "Миниатюры"                           | y |
      | Проверяем у баннера наличие названия                                  | y |
      | Проверяем у баннера наличие скроллера                                 | y |
      | Проверяем количество колонок у товаров баннера с шаблоном "Миниатюры" | 2 |
      | Проверяем у баннера с шаблоном "Миниатюры" количество товаров         | 6 |
    And Делаем скриншот "ProductsAtBanners_Thumbnails_2Columns_InOneRow 01"
    And Переключаемся на "ar" язык интерфейса витрины
    And Скроллимся к блоку с баннером
    And Делаем скриншот "ProductsAtBanners_Thumbnails_2Columns_InOneRow 02(RTL)"