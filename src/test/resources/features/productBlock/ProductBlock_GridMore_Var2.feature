...
Feature: Проверка отображения блока товаров "AB: Сетка с кнопкой "Показать ещё" - Вариант 2
  ...
  Scenario: Работаем с настройками блока товаров "Распродажа"
    ...
    Given Выключаем LazyLoad в секции с блоком "Распродажа" и Включаем отображение для мобильного устройства
  ...
  Scenario: Работаем с витриной и выполняем проверки
    ...
    And Скроллимся вниз по странице на 5 px
    And Делаем скриншот "ProductBlock_GridMore_Var2__0"
    And Выполняем проверки в блоке:
      | Количество колонок в списке | 4 |
    And Нажимаем кнопку 'Показать ещё' в блоке товаров и делаем скриншоты "ProductBlock_GridMore_Var2__"
    And Выполняем проверки в блоке:
      | Отображать пустые звёзды рейтинга товара        | n                     |
      | Отображать общее значение рейтинга товара       | n                     |
      | Отображать кнопку "Добавить в избранное"        | y                     |
    ...
    And Скроллимся к блоку "товаров"
    And Скроллимся вниз по странице на 5 px
    And Делаем скриншот "ProductBlock_GridMore_Var2 (RTL)__0"
    And Нажимаем кнопку 'Показать ещё' в блоке товаров и делаем скриншоты "ProductBlock_GridMore_Var2 (RTL)__"